...
each `create()` result, so multiple physical Skale devices can connect at once.
...
circle button tares only when this device is the current brewing or dosing
role. The square button is active only for the brewing role: it starts from
idle after a guarded inactive-GHC check and stops from espresso through the
guarded machine endpoint. Dosing square presses are ignored.

Changing between the native and plugin driver representations changes the
public device identity. Reselect brewing and dosing device roles after that
...
the current role, public device ID, host connection ID, and role selection ID;
stale sessions and changed settings cancel pending actions.
