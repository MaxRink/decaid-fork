...
The action uses `url_launcher` with `LaunchMode.inAppBrowserView`, falling back
to the platform's normal browser behavior when needed. A failed launch is shown
in the native page. No native settings schema or duplicate persistence store is
created.
