  ...
  Future<Response> _scaleConnectionsHandler(Request request) async {
    return jsonOk({
      'brewing': _scaleConnectionProjection(
        ...
        scale: _connectedScaleOrNull(
          state: () => _scaleController.currentConnectionState,
          connectedScale: _scaleController.connectedScale,
        ),
      ),
      'dosing': _dosingScaleConnectionProjection(),
    });
  }

  Map<String, String>? _dosingScaleConnectionProjection() {
    final controller = _dosingScaleController;
    if (controller == null) return null;
    return _scaleConnectionProjection(
      role: 'dosing',
      generation: controller.connectionGeneration,
      state: controller.currentConnectionState,
      scale: _connectedScaleOrNull(
        state: () => controller.currentConnectionState,
        connectedScale: controller.connectedScale,
      ),
    );
  }

  Scale? _connectedScaleOrNull({
    required device.ConnectionState Function() state,
    required Scale Function() connectedScale,
  }) {
    try {
      if (state() != device.ConnectionState.connected) return null;
